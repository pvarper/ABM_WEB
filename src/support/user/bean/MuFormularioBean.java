...
import support.user.business.FormularioBL;
...
public class MuFormularioBean implements Serializable,IControlPrivilegios {
	...
	private void saveModulo() throws Exception{
		...
		if(!this.edit){
			Integer newId = formularioBL.obtenerNextIdFormulario();
	...
	public void setIdFormMod(Integer idFormMod) {
	...
	public String getNombreMod(){
		if(this.idFormMod > 0){
			for (MuFormulario muFormulario : listMod) {
				if(muFormulario.getId().equals(this.idFormMod))
					return muFormulario.getNombre();
			}
		}
		return "";
